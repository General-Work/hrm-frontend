...
<div
  class:text-sm={textSmall}
  class="flex gap-2 justify-between w-full items-center text-gray-600"
>
  <button
  ...
  <div class="text-sm font-semibold">
    {#if !hideLabel}
      <span>Page</span>
    {/if}
    <span class:hidden={!totalPages} class:flex={totalPages}
  ...
  <button
    class="hover:text-gray-600 font-bold rotate-2"
    class:hidden={!refresh}
    on:click={refresh}
  >
    <iconify-icon
      icon="ph:arrows-clockwise"
      class="hover:text-gray-400 hover:animate-spin"
    />
  </button>
