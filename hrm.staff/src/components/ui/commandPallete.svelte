...
<div
	class="relative z-20 pallete"
	role="dialog"
	aria-modal="true"
	use:hotKeyAction={{ code: 'Escape', cb: () => dispatch('close') }}
>
	<!--
		...
		>
			<div class="relative">
				<iconify-icon
					...
					class="pointer-events-none absolute left-4 top-3.5 h-5 w-5 text-gray-900 text-opacity-40 text-2xl"
				/>
				<input
					type="text"
				...
				{:else if $searchResult.length}
					<ul class="max-h-96 overflow-y-auto p-2 text-sm text-gray-700">
						{#each $searchResult as entry}
							<li class="flex">
								<button
									class="group flex cursor-pointer select-none items-center rounded-md px-3 py-2 hover:bg-gray-900 hover:bg-opacity-5 w-full text-left"
									on:click={() => handle(entry)}
								>
									<iconify-icon
										icon={entry.icon}
										class="text-3xl text-gray-900 text-opacity-40 group-hover:text-opacity-100 group-hover:text-teal-600"
									/>
									<span
										class="ml-3 flex-auto truncate text-opacity-40 group-hover:text-opacity-100 group-hover:text-teal-600"
										>{entry.title}</span
									>
								</button>
							</li>
				...
				{:else}
					<div class="px-6 py-14 text-center sm:px-14" class:hidden={!searchInput}>
						...
						<p class="mt-4 text-sm text-gray-900">
							We couldn't find any actions with this term. Please try again.
						</p>
					</div>
				{/if}
					...
					<li class="p-2">
						...
						<ul class="text-sm text-gray-700">
							{#each $recentSearches as entry}
								<li
									class="group flex select-none items-center rounded-md px-3 py-2 hover:bg-gray-900 hover:text-gray-900 hover:bg-opacity-5 cursor-pointer"
								>
									<iconify-icon icon={entry.icon} class="text-3xl" />
									<span class="ml-3 flex-auto truncate">{entry.title}</span>
									{#if entry.subTitle}
										<span class="ml-3 hidden flex-none text-gray-500 group-hover:block"
											>{entry.subTitle}</span
										>
									{/if}
								</li>
					...
					<li class="p-2">
						...
						<ul class="text-sm text-gray-700">
							{#each $quickActions as entry}
								<li
									class="group flex cursor-pointer select-none items-center rounded-md px-3 py-2 hover:bg-gray-900 hover:bg-opacity-5"
								>
									<iconify-icon
										icon={entry.icon}
										class="text-3xl text-gray-900 text-opacity-40 group-hover:text-opacity-100 group-hover:text-teal-600"
									...
									{#if entry.hotKey}
										<span
											class="ml-3 flex-none text-xs font-semibold text-gray-500 group-hover:text-teal-600"
										>
